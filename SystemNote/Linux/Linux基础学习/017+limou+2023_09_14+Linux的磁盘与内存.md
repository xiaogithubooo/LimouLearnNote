...
>   吐槽：既然操作系统已经有对应的内存保护机制了，为什么 `C/C++` 这类语言会被说为内存不安全语言呢？而主要是操作系统没有办法保证一个进程内部的内存安全，只能保证不同进程的相互之间的内存安全...

## 12.1.分配回收

### 12.1.1.单一连续分配

在单一连续分配方式中，内存被分为系统区和用户区。系统区用于存放操作系统相关数据，用户区用于存放用户进程相关数据。内存中只能有一个用户程序，该用户程序独占整个用户区空间。该分配方法实现简单、无外部碎片，但是已经是历史，只适用于单用户单任务的操作系统，并且有内部碎片。
...
为了支持多道程序并解决程序之间的相互干扰问题，以前的操作系统也曾引入了将用户空间划分为若干个固定大小/不定大小分区的内存管理方式，不会产生内存碎片，这种方式是最早和最简单的多道程序内存管理方式。

并且这种方式需要被操作系统所管理，因此就需要构建对应的分区表，记录分区的大小、起始地址、状态等...

### 12.1.3.动态分区分配

动态分区分配，又称为可变分区分配，是一种更加灵活的内存管理方式。与固定分区分配不同，这种方法不会预先划分内存分区，而是在进程装入内存时，根据进程的大小动态地建立分区，并使分区的大小正好适合进程的需要。因此，系统分区的大小和数目是可变的。

而此时管理动态分区所使用的数据结构可以是空闲分区表或者空闲分区链，内部一定会存储分区的大小和起始位置信息等。并且还有对应的动态分区分配算法，决定一个进程能使用的内存区。

这种分配方式不会产生内部碎片，有可能产生外部碎片，但是我们一般担忧的都是内部碎片的问题...

>   补充：实际上释放内存就是把表项进行对应的修改，例如把大小减少、置为 `0`、删除对应的表记录、合并等方式...

#### 12.1.3.1.连续分配管理方式

-   首次适应算法，每次都从低地址开始查找，找到第一个满足大小的空闲分区（其实就是查询空闲内存分区表），这个分区表是按照地址排列的。
-   最佳适应算法，把空闲分区表按照空间大小升序排列，寻找最小适应的内存分区在进行分配。
-   最坏适应算法，就是把最佳适应算法变为降序查找了，可以避免碎片变多
