...
>
>        (3)使用锁的时候允许因为等级原因进行剥脱（如果一个优先级别高的线程急需使用锁，可以把持有锁的其他线程的锁强训释放掉，但由于是强制执行，操作不得当很容易出现问题，也就是 **释放他人的持有锁**）
>
...
这个模型会有一种特殊的情况，假设所有的哲学家都拿自己左手边（或右手边）的一把叉子，此时就会出现死锁的问题。那么如何设计算法，使得每位哲学家都能够进餐，但又不会发生死锁呢？

-   一种解决方案是引入一个策略，即当哲学家要进餐时，他需要同时获取左右两边的叉子。如果叉子被其他哲学家占用，他就需要等待直到两边的叉子都可用，而不能只拿一把叉子。这样一来，每位哲学家都不会陷入死锁状态，因为每位哲学家都会及时释放叉子。
-   另外一种解决方案是引入一个“服务生”，服务生负责管理叉子的分配。当哲学家想要进餐时，他需要向服务生请求叉子，服务生会确保两把叉子被分配给哲学家。这种方法也能够避免死锁的情况。
