...
>   ==产生死锁的现象==
>
>   死锁有一种很形象的理解方法，如果两个车辆在一个狭窄的桥中相对相遇，此时 `A` 车认为应该等待 `B` 车先过才能过去，而 `B` 车认为应该等待 `A` 车先过才能过去，结果造成了互相等待，这种现象就很类似死锁。
>
...
>   4.   **循环等待条件**：若干执行流之间形成一种头尾相接的循环等待资源关系，也就是申请了其他执行流正在使用的资源
>
>   ==避免死锁的产生==
>
>   1.   **静态策略：破坏产生死锁的四个必要条件**
>
>        (1)简单除暴不加锁，就没有死锁的问题，因为有些代码实际上不需要加锁。
>
>        (2)使用锁的时候允许因为等级原因进行剥脱。如果一个优先级别高的线程急需使用锁，可以把持有锁的其他线程的锁强训释放掉。但由于这是强制执行，操作不得当很容易出现问题。
>
>        (3)保证资源一定会被释放。如果在获取其他锁失败时，释放之前拥有过的锁。
>
>        (4)没有循环等待资源关系，规范使用锁。资源一次性分配，不要遇到一个地方就加个锁，尽量一次性加锁获取资源
>
>   2.   动态策略：安全序列、不安全状态、银行家算法。安全序列就是指把资源按照一定顺序进行分配，保证不发生死锁的分配序列。如果找不出这一序列就会进入不安全状态，此时就有可能发生死锁请求，这也是银行家算法的核心思想（后面我会提到）。
>
>   ==避免死锁的算法==
>
>   1.   **死锁检测算法**
...
>   3.   ...
>
>   另外，在实践中基本能不加锁就不加锁，在无需求的情况下加锁会导致代码复杂性提高，导致未来无法偿还的技术债务。关于死锁，未来我可能还会出专门的专题来解答，待补充...
>
...
![image-20240613125237265](./assets/image-20240613125237265.png)

![image-20240613125312571](./assets/image-20240613125312571.png)

![image-20240613125411811](./assets/image-20240613125411811.png)

![image-20240613125420515](./assets/image-20240613125420515.png)

更快的

![image-20240613125529605](./assets/image-20240613125529605.png)

![image-20240613125557339](./assets/image-20240613125557339.png)

实现

![image-20240613125747803](./assets/image-20240613125747803.png)

检测还有资源图https://www.bilibili.com/video/BV1YE411D7nH?p=41&vd_source=c92c89dbfcf9cc30c48086469621f35b
