...
3. 退出：`esc+:+wq`这里`w`是保存，`q`是退出（加`!`是强制执行），连用就是保存文件并且退出vim。

# 2.Vim普通模式（Normal Mode）
...
# 5.Vim替换模式（Replace Mode）

就是之前提到的`R`。

# 6.Vim视图模式（Visual Mode）
