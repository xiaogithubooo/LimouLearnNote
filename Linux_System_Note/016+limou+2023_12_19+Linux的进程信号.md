...
1.   我们可以识别出信号，知道后续对应的执行动作
2.   即使没有收到特定的信号，我们也知道当下应该继续做什么
3.   收到信号后，不一定就要立刻执行，需要先被临时记忆/临时存储起来
...
以上代码子进程退出了，发送给父进程 `17` 号信号，该信号默认为忽略，而父进程还在执行自己的代码，因此子进程变为僵尸进程。
...
欸，我们仅仅是增加了`signal(SIGCHLD, SIG_IGN);`这句代码，为什么子进程就会自己释放自己了呢？并且`17`号信号本身的行为不就是忽略么，为什么再次设置忽略了？
