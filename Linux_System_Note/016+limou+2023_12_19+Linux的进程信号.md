...
其中 `[1, 31]` 为普通信号，最为常用，后面 `[34, 64]` 为实时信号，比较少用，除非是某些特殊的行业（比如车载操作系统）。特定信号的详细描述则可以使用指令 `man 7 signal` 来查看：
...
3.   **Ign（Ignored）**：表示信号被忽略，系统不会采取任何处理动作
4.   **Stop（Stop）**：表示表示停止，用于暂停一个进程的执行，使其进入停止状态，此时进程会被挂起
5.   **Cont（Continue）** ：在进程信号中通常表示继续执行，这个信号用于恢复一个之前被停止的进程的执行，当一个进程收到停止信号（如 `SIGSTOP`）而被暂停后，可以通过发送 `SIGCONT` 信号使其继续执行

>   补充：“进程基础”中，我们曾经提及到“...还有 `1` 个比特位是 `core dump` 标志，这个我们之后讲信号再来谈及（是否具备 `core dump`）...”，也就是这里提到的“核心转储”，那么这个标志位有什么用呢？
>
...
>
>   而某些信号的行为会导致核心转储的启用，可以帮助开发人员更好确定错误...
...
3.   **如何理解组合键 [ctrl+z] 变成信号呢？**
     ...
     >   >   补充 `1`：中断机制
     >   >
     >   >   中断的概念实际是计算机组成原理中常见的知识，当一个中断事件发生时，`CPU` 会立即停止执行当前的指令序列，保存当前执行上下文的状态，然后转向处理中断的代码。处理中断的代码通常由操作系统内核中的中断服务例程（`ISR`，`Interrupt Service Routine`）来处理。
     >   >
...
使用过 `signal()` 后，这里有几个问题我先提出来：

1.   我们可不可以把所有的信号的捕捉方法全部修改，让一个进程无法退出只能持续运行？（吐槽：这不就是妥妥电脑病毒么）
...
我们可以看到除了 `9)SIGKILL` 信号，其他的信号都能被修改处理行为，为什么 `9)SIGKILL` 不可以修改呢？因为该信号属于管理员级别的信号，无法使用系统调用做修改，因此这种恶意程序是不被操作系统允许的。
...
我们发现 `9)SIGKILL` 信号依旧是比较特殊的，我们无法对其进行屏蔽，那我们就重新设计一个 `shell` 脚本来跳过 `9) SIGKILL` 信号（实际上 `19)SIGSTOP ` 信号也一样是特殊的，我们也将其屏蔽）
...
最终结论：`9)SIGKILL` 信号极度特殊，我们无法使用接口阻塞和屏蔽。
