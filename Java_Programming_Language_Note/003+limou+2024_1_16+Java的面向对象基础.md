...
```java
// demo1 包
package com.limou.blog.demo1;

public class Computer {
    String brand; // 品牌（默认限定符为 default）
    private String cpu; // cpu（私有）
    private String memory; // 内存（私有）
    public String screen; // 屏幕尺寸（公有）
...
import com.limou.blog.demo1.Computer;
public class TestComputer {
    public static void main(String[] args) {
        Computer p = new Computer("DELL", "i5", "16G", "2240*1400");
...
使用 `{}` 定义的一段代码被称为代码块，根据代码块定义的位置和关键字，可以分为：
