...
-   **路径和路径长度（无权路径长度、带权路径长度）**：路径是指一个顶点$v$可以到达另外一个顶点$v'$之间的顶点序列$V = \{v = v_{i}, v_{i+1}, v_{i+2}, ..., v_{i+n} = v' \}$，而路径长度就是一条路径上经过的边或者弧的数目，也就是$V.size() - 1$，如果顶点序列没有出现重复的元素，就是一个简单路径。
    ...
    (2)带权路径长度：若路径长度要考虑带权值，就是带权路径长度，结果为路径上所有权值之和（相当于 $1×权_{1} + 1×权_{2} + ... + 1×权_{n-1}$）
...
\begin{align}
ve(0) &= 0\\
ve(i) &= Max\{ve(k) + w_{k,i}\}, k为i的上一个顶点\\
\end{align}
$$
...
>   经验：某顶点的“事件最早发生时间=0+该顶点为终止的最大带权路径”
...
\begin{align}
vl(n-1) &= ve(n-1)\\
vl(i) &= Min\{vl(k) - w_{i,k}\}, k为i的下一个顶点\\
\end{align}
$$
...
>   经验：某顶点的“事件最晚发生时间=整个图最大带权路径-该顶点为开始的最大带权路径”
...
$$
\begin{align}
e(i) &= ve(j),j 为 ai 的边或弧尾\\
\end{align}
$$


>   补充：某活动的“活动最早开始时间=边或弧尾顶点的事件最早发生时间”
...
>   补充：某活动的“活动最早开始时间=边或弧头顶点的事件最晚发生时间-活动权值”
