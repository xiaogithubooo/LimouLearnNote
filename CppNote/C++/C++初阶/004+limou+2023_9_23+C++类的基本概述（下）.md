...
int main()
{
    ...
    Data* p1 = &d1;
    Data* p2 = &d2;
    cout << Less(p1, p2) << endl;//调用特化后的函数模板
    return 0;
}
```
...
2.   在编译阶段，会对多份源文件做各自的编译（进行词法、语法、语义分析、错误检查等）并且生成多份的汇编代码（注意头文件是不会参与编译的）这个时候在`function.obj`或者说`function.o`中，由于编译器没有看到函数的实例化，因此没有生成具体的加法函数。而在`main.obj`或者`main.o`中，编译器看到有加法函数的调用，但是暂时不知道具体的实现，因此就暂时放进了符号表里等待后续链接
3.   在链接阶段由于没有实例化，因此`function.obj`或者说`function.o`中没有加法函数的定义，根本就无法提供加法函数的地址在符号表里供`main.obj`或者`main.o`链接
...
2.   在一个翻译单元里分离（即：干脆直接将定义和声明都写在一个`.hpp`内，这样是更加推荐的）。
