...
但是但是，`C++`有些例外情况，使得哪怕函数签名不完全相同也构成多态：

1.   使得虚函数的返回值可以不同，这种情况也叫做”协变“，但是此时的返回值必须是具有父子继承关系类的类型的指针或引用（如果是其他不具有继承关系的类型的返回值就有可能报错，并且注意这里的父子类型只要是继承的父子就可以，这）。

并且如果父类的函数加了`virtual`成为了虚函数，那么子类就可以不加`virtual`，依旧是可以重写（挺让人吐槽的），但是我们建议加上，这样可读性更好。
