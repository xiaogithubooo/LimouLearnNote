...
```cpp
// muduo::net::TcpServer(TCP 服务器类)
typedef std::shared_ptr<TcpConnection> TcpConnectionPtr;
typedef std::function<void (
    const TcpConnectionPtr& // TCP 连接对象
)> ConnectionCallback;
...
public:
    ...
    void setThreadNum(int numThreads); // 设置线程池的线程个数
    void start(); // 启动服务器, 开始套接字监听
    void setConnectionCallback(const ConnectionCallback& cb); // 新连接变动时被自动调用(回调函数)
    void setMessageCallback(const MessageCallback& cb); // 新连接收到新消息时被自动调用(回调函数)
}
```
...
public:
    ...
    // Get the TcpConnection object, might not be established yet
    ...
    // Set the callback function for receiving messages from the server
...
/*
Note: Since the muduo library operates asynchronously for both server and client,
sending data before the connection is fully established is not allowed.
We can use the built-in CountDownLatch class for synchronization control.
*/
...
public:
    ...
    // Decrease the count, notify all when it reaches zero
    void countDown() {
        MutexLockGuard lock(mutex_);
        --count_;
        if (count_ == 0) {
