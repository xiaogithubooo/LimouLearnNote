...
上述三种应用场景，理论上应该是有三种接口，但是 `Linxu` 上统一使用同种接口，而我们重点学习最后一种网络套接字。
...
>   补充：`套接字 API` 是一层抽象的网络编程接口, 适用于各种底层网络协议。然而，各种网络协议的地址格式并不相同，其中参数 `sockaddr` 的结构有如下三种选择：
>
>   ![image-20240209223429854](./C:/Users/Limou_p350ml9/AppData/Roaming/Typora/draftsRecover/assets/image-20240209223429854.png)
>
>   1.  **`struct sockaddr`**：通用的抽象结构，最原始的结构
>   2.  **`struct sockaddr_un`**：是用于 `UNIX` 域套接字的结构体
>   3.  **`struct sockaddr_in`**：用于 `IPv4` 网络地址的结构体（我们重点xue'zhe'fe）
>
...
![image-20240314000339847](./C:/Users/Limou_p350ml9/AppData/Roaming/Typora/draftsRecover/assets/image-20240314000339847.png)
