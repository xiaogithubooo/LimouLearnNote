...
>   补充：`套接字 API` 是一层抽象的网络编程接口, 适用于各种底层网络协议, 如 `IPv4`、`IPv6`，以及后面要讲的 `UNIX Domain Socket`. 然而，各种网络协议的地址格式并不相同，其中参数 `sockaddr` 的结构有如下三种选择：
>
...
```cpp
//UDP 的数据读写接口（由于 UDP 是面向数据报的，因此就需要特别于字节流的都且接口）
#include <sys/types.h>
#include <sys/socket.h>
...
```cpp
//TCP 的数据读写接口（由于 TCP 是面向数据流的，还需要更多的安全连接工作在使用和文件 IO 一样的读写接口）
...
subgraph client绑定套接字
    ...
    auto["OS 在 sendto() 自动随机绑定，无需 bind()"]
    socket --> auto
end
...
使用上述 `API` 来做一个 `CV` 框架，并且先制作出 `makefile` 方便后续的自动化运行（注意，内部对应的头文件和源文件您需要自己提前创建和写好 `main()` 函数）。
...
然后使用 `./udp_server 0.0.0.0 8080` 即可运行服务端程序，并且可以使用 `netstat -anup` 检验服务端是否可以成功启动。
...
可以注意到，现在的服务器是永远不会退出的（在 `Start()` 中使用了 `while(true);`），这种进程就是常驻进程，直到服务器宕机。

这种程序也是最担心内存泄漏的，哪些只需跑一次的程序哪怕真的内存泄漏了，通常也没有太多的影响。

前面我们只是完成了服务端和套接字的初始和绑定工作，接下来我们进一步写一下服务端的读写服务，并且规范了一些写法。
...
接下来，我们在这份代码的基础上进行修改，创造出更多常见的网络应用。
...
![image-20240316140630262](./assets/image-20240316140630262.png)

![image-20240316140406770](./assets/image-20240316140406770.png)

![image-20240316140433022](./assets/image-20240316140433022.png)
...
![image-20240317104153520](./assets/image-20240317104153520.png)
...
上述代码只修改了服务端的代码，这是利用操作系统托管孤儿进程的机制，算是一种取巧的方式。
...
>   注意：上述代码貌似还有点小问题，没时间 `debug` 了以后再来重新看看，重新补一下录播，并且这次需要把一些接口的返回值看清楚一些，并且把之前的三子棋代码找回来，重新编写。
...
1.   `TCP` 是面向连接的通信协议，在内部会自动进行“三次握手”来建立连接，这个过程是在调用 `connect()` 后，发起的，但是 `accept()` 本身不参与“三次握手”，该过程由操作系统来自动完成，只是“三次握手”结束后，`accept()` 才把建立好的连接从内核中返回到用户层中（也就是返回一个文件描述符 `fd`），而通过这个文件标识符就可以进行 `IO` 操作。

     而两个端都使用 `close(fd)` 就是断开连接，而断开连接时就会释放连接资源，会导致“四次挥手”的行为，该行为需要双方共同协作，而不是仅一方主动。

     >   `CS` 模型中一般总有一方时主动访问，一般是客户端主动向服务端进行访问。

2.   而 `UDP` 的可靠性（也就是丢包率）现在我们也可能很难测试出来，尽管 `UDP` 有可能出现丢包和不会重传包。

     有链接和无连接我们在代码中就可以看到，而数据报（发一条消息得一条消息）和字节流（只有到读的时候才把数据一次读走）
