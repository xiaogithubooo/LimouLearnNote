...
```mermaid
flowchart TD
tcp["TCP 协议下的套接字编程"] --> server & client
...
class UdpServer
{
    ...
    public:void Start()
    {
        while (true)
        {
            ...
            if ( sendto(_sock, readBuff, strlen(readBuff), 0, (struct sockaddr*)&peer, peerLen) > 0 ) //读取成功，注意这里是 strlen()，只发送有效的数据，并且前面读取数据的时候也获取了客户端的套接字消息，可以直接在这里使用
            {
                LogMessage(NORMAL, "server write done ... %d %s %s %d", errno, strerror(errno), __FILE__, __LINE__);
            }
            else //读取失败
...
>
>   在使用 `popen()` 打开一个管道时，管道中的进程会在读取到数据之前一直等待。这就解释了为什么在执行 `popen()` 后，`wc -l` 命令会等待三行输入。
>
>   使用了 `fprintf()` 向管道写入了三行数据，然后关闭了管道。这三行数据会被传递给 `wc -l` 命令，但 `wc -l` 会等待更多的输入，直到管道被关闭。
>
        ...
        while (true)
        {
            ...
            if (recvfrom(_sock, readBuff, sizeof(readBuff) - 1, 0, (struct sockaddr*)&peer, &peerLen) >= 0) //读取成功
            {
                ...
                else //非危险指令
                {
                    ...
                    {
                        LogMessage(FATAL, "%s popen() error %d %s %s %d", readBuff, errno, strerror(errno), __FILE__, __LINE__);
                        exit(50);
                    }
                    else //管道创建无错
                    {
...
int main(int argc, char* argv[]) //./udp_client 127.0.0.1 8080
{
    ...
    while (true)
    {
        ...
        //实际上我们已经有了服务端的 ip 和 port 的信息了，这里我只是为了占位和调用规范写的，在本项目中可以不怎么处理（不过有时客户端可以是其他端的客户端，因此可能需要用到这个）
        struct sockaddr_in temp;
        bzero(&temp, sizeof(temp)); //比特位置零
        socklen_t len = sizeof(temp);
        if ( recvfrom(sock, readBuff, sizeof(readBuff) - 1, 0, (struct sockaddr*)&temp, &len) >= 0 )
