...
您还可以结合我们之前写的生产者消费者模型，把服务端的读和写进行分离，一个线程从客户端接收到消息存储到 `std::queue<std::string> messageQueue;` 中，再由另外一个进程从队列中拿数据进行发送。不过这里我暂时不写，您有时间可以试一下，我将在后续的 `TCP` 编程中演示。
...
>   补充：`TCP` 的“面向链接”就体现在使用接口 `listen()` 和 `accept()` 上。
