...
```mermaid
flowchart TD
A["本端"] --> B["设置运行的 ip 和运行的 port"] -->|"从主机字节序"| C["htons()/htonl()"] & D["inet_addr()"] -->|"转化为网络字节序发送出去"| E["对端"] -->|"从网络字节序"| F["ntohs()/ntohl()"] & G["inet_ntoa()"] -->|"转化为主机字节序"| H["接受"]
```
...
>   注意：当然目前的服务端还是不支持类似 `vim` 和 `top` 等指令，原因是这些指令都需要进行交互。
...
uint16_t serverport = ...; //这里改成您服务端程序所占 port 号
std::string serverip = "..."; //这里改您服务端程序所占 ip 地址
