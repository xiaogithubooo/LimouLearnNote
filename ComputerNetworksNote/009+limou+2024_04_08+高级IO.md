...
-   **ET 模式**：而边缘模式下的描述符就绪时，就必须立刻处理，虽然只读取了部分数据，但是第二次调用时，`epoll_wait()` 不会再返回该就绪描述符，因此需要注意就绪描述符丢失的问题。很多知名的服务器就会采用这种模式（例如 `Nginx`），该模式支持非阻塞读写，且代码编写的复杂程度较高（这个模式最好使用非阻塞的 `IO`）。
...
接下来，我们来尝试编写一个较为高性能的 `IO` 服务，加入对描述符的 `ET` 模式设置、写事件的动态关注、自定义协议、业务逻辑、端口复用等。有了这个基本框架，您完全可以修改出一个 `Epoll-ET-Http-IO-Server` 服务器。
