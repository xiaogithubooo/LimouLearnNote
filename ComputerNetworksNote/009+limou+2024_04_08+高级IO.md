...
int select(
    ...
    struct timeval* timeout // 阻塞模式/等待策略
); // 返回就绪的描述符个数(数据就绪或空间就绪)
// 输入就是用户输入提醒系统需要关注的事件
// 输出就是系统输出提醒用户哪些事件已就绪
```
...
int main()
{
    ...
    int n = gettimeofday(&currtime, nullptr); // 获取当前地区的时间, currtime 中包含时间戳秒和时间戳微秒
...
-   `timeout=nullptr`：阻塞式，描述符准备好就返回
-   `timeout=(0, 0)`：非阻塞式，描述符没准备好也返回
-   `timeout=(X, Y)`：在 `X’Y’’` 时间内阻塞，但时间到就返回准备好的描述符

我们先来体验一下多路转接阻塞的感觉，您会发现等待的操作由 `select()` 一个接口来承担，尽管需要您调用读写接口进行 `IO` 但是 `select()` 可以保证在 `IO` 时不必过多等待，一点是处于资源就绪的情况（有空间可写、有数据可读）。
