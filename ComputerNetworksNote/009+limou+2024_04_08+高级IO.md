...
>   总结：很明显，`poll` 服务器和 `select` 服务器的编写过程是很相似的，优点也类似，也是适应少量活跃大量链接的应用场景（我们实际上没改什么代码就可以把 `select` 服务器改造为 `poll` 服务器）。但是代码编写过程相对简单，理论上没有 `fd` 的管理上限，并且参数做了较好的分离，还添加了更多的事件功能。并且无需再计算最大的描述符了，只需要规定一个存储描述符的最大数组大小。
>
>   但是 `poll` 的缺点也有：
>
>   -   依旧需要遍历，这点和 `select` 服务器是一样，这是主要问题。尤其是在描述符的分布比较离散时，不仅需要用户维护数组，还可能需要较大程度的遍历
...
`epoll` 的 `e` 实际上就是 `enhance` 的意思。是官方为了处理大批量句柄（标定特点文件资源的数据对象）且优化过的 `poll`。但是我们实际上会发现 `epoll` 的使用过程和 `poll` 差别很大，甚至可以说是两个东西（本质是因为底层原理不一样，学习 `epoll` 的底层原理比 `epoll` 本身更为重要）。与之相关的接口如下：
