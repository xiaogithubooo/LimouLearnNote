...
>   1.  **阻塞 IO**：在阻塞 IO 模式下，当应用程序发起一个 IO 操作时，该操作会一直阻塞直到完成，导致应用程序无法执行其他任务。
>   2.  **非阻塞 IO**：非阻塞 IO 模式下，应用程序发起一个 IO 操作后，会立即返回，无需等待 IO 操作完成。应用程序可以继续执行其他任务，随后通过轮询或者事件驱动等方式来查询 IO 操作的状态，以判断是否已经完成。
>   3.  **异步 IO**：在异步 IO 模式下，应用程序发起一个 IO 操作后，不需要等待操作完成，而是继续执行后续的任务。当 IO 操作完成时，系统会通知应用程序，应用程序再去处理已完成的 IO 操作。这种方式下，应用程序不需要主动轮询或者等待 IO 操作的完成。
