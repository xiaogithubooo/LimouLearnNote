...
外键的语法为 `FOREIGN KEY field_name REFERENCES main_table_field_name;` 有些时候，表与表之间是会产生关系/依赖的。因此外键和之前所有的键不太一样，是根据其他数据表来产生约束的。
...
mysql> delete from class where c_id=3;
Query OK, 1 row affected (0.00 sec) # hen'm'x
```
