...
>   补充：更新和替换的区别
>
>   最为本质的就是“更新”是遇到冲突时，更新数据。而“替换”时遇到冲突时，先删除旧数据再插入新数据。
>
...
# (3)会删除 table_name 表中的所有行
DELETE FROM table_name;
# 尽管这个操作会清空表中的数据，但它不会重置表的自增 ID 计数器（如果有的话）。如果表中有外键约束和触发器，DELETE 语句会触发这些约束和触发器。DELETE 操作通常比 TRUNCATE 操作慢，因为它逐行删除数据，并且记录了每一行的删除操作，以便可以回滚。
# (4)会重置 table_name 表中的所有行
TRUNCATE TABLE table_name;
# 会快速移除表中的所有行，并且自动重置任何自增的 ID 计数器。相对于 DELETE，TRUNCATE 是一个更快的操作，因为它不逐行删除数据，而是直接重新初始化表。然而，TRUNCATE 不能触发表的删除触发器，并且在某些情况下可能不受外键约束的限制。
```
...
而检索出的结果有可能还需要进一步筛选，这个使用就要使用 `WHERE` 条件子句。
