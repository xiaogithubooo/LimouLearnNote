...
INSERT INTO table_name field_name[,...] VALUES (value[,...]); # 插入一行数据进入表格
...
DELETE FROM table_name WHERE condition; # 删除数据表的某一行/记录

DELETE FROM table_name;
# 会删除 table_name 表中的所有行。尽管这个操作会清空表中的数据，但它不会重置表的自增 ID 计数器（如果有的话）。如果表中有外键约束和触发器，DELETE 语句会触发这些约束和触发器。DELETE 操作通常比 TRUNCATE 操作慢，因为它逐行删除数据，并且记录了每一行的删除操作，以便可以回滚。
