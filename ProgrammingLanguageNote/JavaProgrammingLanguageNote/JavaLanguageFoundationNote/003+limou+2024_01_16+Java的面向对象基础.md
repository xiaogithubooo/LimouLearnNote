...
因此就诞生了 `static` 修饰的成员变量，也被称为“静态成员变量”（沿用 `C/Cpp` 的说法），该成员不属于任何一个具体的对象，是被所有对象共享的，也是单属于类的属性。
...
-   可以省略创建对象的过程，直接使用类名来使用静态函数，使得代码更加整洁。因此适用于一些不需要依赖具体对象的方法，使用起来更加符合语义，更适合作为全局函数
-   静态访问问题...待补充...
