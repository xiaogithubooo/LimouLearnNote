...
bind 0.0.0.0
port 26379
sentinel monitor redis-master redis-master 6379 2 # sentinel monitor 主节点名 主节点ip 主节点端口 法定票数
...
> [!IMPORTANT]
>
> 补充：上面的配置文件有些许需要注意的地方。
>
> - 由于使用 `docker` 的编排容器，可以直接写 `docker` 的容器名, 会被自动 `DNS` 成对应的容器 `ip`
> - 哨兵需要判定主节点是否挂了，但是有的时候可能因为特殊情况, 比如主节点仍然工作正常，但是哨兵节点自己网络出问题了，无法访问到主节点了，此时就可能会使该哨兵节点认为主节点下线，出现误判，使用投票的方式来确定主节点是否真的挂了是更稳妥的做法。需要多个哨兵都认为主节点挂了，`票数 >= 法定票数` 之后，才会真的认为主节点挂了
> - 主节点和哨兵之间也是通过心跳包来进行沟通，如果心跳包在指定的时间内还没回来，就视为节点故障。`sentinel down-after-milliseconds` 则用于指定 `Sentinel` 判断主节点或从节点宕机所需的时间，`Sentinel` 在这个设置时间内无法在节点处收到正常响应时，就会认为该节点“下线”。

##### 12.3.2.3.主从编排

```shell
# redis-work-node-compose.yml
$ sudo vim /etc/redis/redis-work-node-compose.yml
$ sudo cat /etc/redis/redis-work-node-compose.yml
services:
  master:
    image: 'redis:latest'
...
```shell
# redis-sentinel-node-compose.yml
$ sudo vim /etc/redis/redis-sentinel-node-compose.yml
$ sudo cat /etc/redis/redis-sentinel-node-compose.yml
services:
  sentinel1:
    image: 'redis:latest'
...
```shell
# 启动指令
docker-compose -f redis-work-node-compose.yml up
docker-compose -f redis-sentinel-node-compose.yml up
