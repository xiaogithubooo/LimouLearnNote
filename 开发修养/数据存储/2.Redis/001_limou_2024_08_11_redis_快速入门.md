...
```shell
# 复制的建立
$ redis-server /etc/redis/redis-slave.conf --port 6380 --slaveof 127.0.0.1 6379 # 6379 的主节点就连接上 6380 的次节点
...
在从节点中可以执行 `slaveof no one` 断开和主节点的复制关系，从节点就会晋升为新的主节点，断开复制节点后不会抛弃原有数据，只是无法再获取主节点上的数据变化。
