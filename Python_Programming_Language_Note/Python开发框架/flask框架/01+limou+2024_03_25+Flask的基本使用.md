...
另外，`Flask` 默认给我们分配的 `ip` 和端口号是 `127.0.0.1:5000`，我们可以进行手动修改，同样是在 `app.run()` 中使用参数 `host='ip地址'` 和 `port='port端口号'`。一般我们在实际开发中 `ip地址` 设置为 `0.0.0.0` 表示服务端进程使用本主机上的任意一个网卡的 `ip地址`，而端口号一般设置为 `8` 开头的 `4` 位数字。
