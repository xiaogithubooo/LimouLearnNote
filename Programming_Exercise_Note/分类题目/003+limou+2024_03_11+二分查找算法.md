...
public:
    int search(std::vector<int>& nums, int target)
    {
        ...
        while (left <= right) //等于是为了处理特殊情况（只有一个数的情况），也可以说是多次二分后剩下一个数的q'kuang
        {
...
先对题目的情况进行分析，序列本身时具有二段性（可以将序列划分为 `[{target<0}],[{targe>=0}]`），因此可以考虑使用二分法找出左边界。每次如果 `middle<target` 就 `left=middle+1`，如果 `middle>=target` 就 `right=middle`：

![image-20240312124202373](./assets/image-20240312124202373.png)

最后我们会发现，无论是序列中有结果、还是序列中的值都大于 `target`、亦或者序列中的值都小于 `target`。在 `left` 和 `right` 相等时就找到了边界（后两种情况就需要额外判断，如果是都小于那么就会一直），因此我们的循环条件只用写 `left<right` 即可。
...
但是我们也可以选择使用二分法来解决，假设我们要求 `9` 的平方根，这个时候我们首先可以确定，其平方根结果 `3` 一定在 `[1,9]` 内
