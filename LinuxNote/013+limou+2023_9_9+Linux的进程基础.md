...
3. 还有 `1` 个比特位是 `core dump` 标志，这个我们之后讲信号再来谈及
...
由于子进程也是进程，因此在下面图解中，我描绘了一个单独进程调用 `execl()` 时发生的过程。
...
无论是什么语言最后都会变成进程，因此包含上述调用的代码就可以做到调用其他语言代码生成的进程（不单单是其他的系统命令程序），这就意味着可以多代码合作。

而使用 `exec` 系列的函数实际上就是进程加载到内存的过程（将进程从磁盘加载到内存），也就是加载器最重要的功能！并且应该是先创建出进程对象，然后再加载的（因为有可能暂时还不用运行进程，只需要有就行）。

另外，上述函数都不是严格意义上的系统接口，真正的系统接口是 `execve(const char* filename, char* const argv[], char* const envp[]);`，而其他调用都是这个系统接口的派生函数。

>   补充：还有一个 `execvpe()`，为了避免记忆混乱，这里单独列出这个函数。
>
>   ```cpp
>   #include <unistd.h>
