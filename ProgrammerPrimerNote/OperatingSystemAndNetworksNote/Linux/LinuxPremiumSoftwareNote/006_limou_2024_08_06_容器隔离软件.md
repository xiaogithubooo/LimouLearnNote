...
-   **[version]:** `Compose` 不再使用 `version` 来选择一个确切的模式来验证 `Compose` 文件
...
另外，可以通过配置 `.env` 文件来让 `docker compose` 来读取变量。
...
services: # 设置顶级元素 service
  ...
  container2: # 第二个服务
    ...
    volumes:
      - /home/ljp/data/docker-compose/docker2:/home:rw # 挂载目录
  ...
  container3: # 第三个服务
    ...
    volumes:
      - /home/ljp/data/docker-compose/docker3:/home:rw # 挂载目录
