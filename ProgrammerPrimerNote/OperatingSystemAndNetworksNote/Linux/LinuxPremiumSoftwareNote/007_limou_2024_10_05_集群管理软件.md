...
> 补充：两种服务发现类型的区别是什么？
>
> -   `NodePort` 是通过节点端口暴露服务，适用于开发、测试，或者对外暴露服务的简单需求。
> -   `LoadBalancer` 是通过负载均衡器暴露服务，适用于生产环境，提供更强的自动化、负载均衡和高可用性。
>
> 在生产环境的 `Kubernetes` 集群中，`NodePort` 和 `LoadBalancer` 之间的区别更加明显，待补充...
