...
简单来说 `minikube` 通过一个镜像文件创建了一个模拟物理机的虚拟 `Node`，而该 `Node` 容器内部又运行多个 `Pod` 容器模拟 `k8s` 部署中的核心组件，原本这些核心组件应该运行在别的物理主机，而不应该一起被放在本 `Node` 中和其他 `Pod` 塞在一起（看起来还挺绕的，可以补充一张图，待补充...）。

这样用户不需要一上来就解决 `k8s` 的核心组件问题，而是直接创建控制器，进而直接创建 `Pod` 节点来使用。
...
# 也可以使用下面的指令(这是官方推荐的)
$ minikube service hello-node # 在 Ubuntu24.04LTS 环境下可以自动打开上述的网站
