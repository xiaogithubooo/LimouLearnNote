...
如果是在不同分支里修改了相同的文件，就会发生冲突，这个时候两份文件就会直接合并在一起，需要我们手动修改冲突的地方并且重新 `add` 和 `commit` 一次。
...
$ git add --all && git commit -m "dev: 改动"
[dev bc856f4] dev: 改动
...
$ git add --all && git commit -m "master: 改动"
[master f8aa0c7] master: 改动
 1 file changed, 3 insertions(+)

# 此时产生冲突
$ git log --oneline --graph --all
* f8aa0c7 (HEAD -> master) master: 改动
| * bc856f4 (dev) dev: 改动
|/  
* 3657ef8 删除文件
* f64435b 这是第一次填写推送信息记录, 要认真写, 方便日后回溯
...
合并是将一个分支的更改合并到另一个分支中 `git rebase master [某分支名]`（之前是把某分支合并到本分支，这里是把本分支变基到某分支，这是相反的）。
