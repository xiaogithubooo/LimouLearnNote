...
$ # 程序员编写代码产生某文件
$ git add [某文件] # 添加到暂存区
$ git commit -m "填写推送信息记录, 要认真写, 方便日后回溯" # 添加到存储库

```

## 3.2.工作状态

使用 `status` 可以查看当前工作目录（或者叫工作树）中有哪些文件没有被 `add`，那些文件等待 `commit`。
...
```bash
$ git log [--oneline] # 可以加上参数 "--oneline" 试试, 输出更加简洁明了
```
...
> 补充：查看哈希值。
>
> - 使用 `git log` 可以查看所有提交的哈希值。用 `git ls-tree <HEAD | 特定commit的哈希值> <file_path>` 可以查看当前版本的文件的哈希值
...
这里给出一个使用 `diff` 的例子，根据下面提供的输出，可以看到在 `git diff` 命令的结果中，显示了代码文件 `test.c` 的变化。
...
为什么范围是 `5` 行而不是 `2` 行呢？这是因为 `Git` 使用上下文行来提供更多的变更上下文和可读性。当显示 `diff` 输出时，`Git` 通常会包含上下文行（`context lines`），以便更好地理解变更的上下文，并在合并等操作中提供更好的决策（因此显示多少取决于 `Git` 的判断，但是 `Git` 可以保证您阅读清晰）。
...
1. `--soft` 参数，将 `版本库` 回退到某个指定版本，其他保持不变
2. `--mixed` 参数（默认），将 `版本库` 回退到某个指定版本，将 `暂存区` 的内容退回工作区，保留工作区的改动
3. `--hard` 参数，将 `版本库` 回退到某个指定版本，清空暂存区和工作区所有更改（使用该参数需要谨慎）

> [!IMPORTANT]
>
> 补充：`HEAD` 的使用有点类似绝对路径和相对路径的区别，这个参数可以直接写成某次 `commit` 的哈希值，表示回退到指定的版本。也可以使用 `^` 来回退，`HEAD` 表示当前版本，`HEAD^` 表示上一个版本，`HEAD^^` 表示上上个版本...
...
可以使用 `git reflog`，这条指令会从本地克隆或创建仓库开始，记录本地的每一次提交命令以及重要的 `commit`（不过也仅限于本地）。
...
在进行 `add` 后，如果此时只有工作区内部修改了该文件，又觉得在工作区的修改不够满意，只想将工作区回退，可以使用 `git checkout -- <文件路径>`，回退到最近一次的 `add`，也就是专门丢弃工作区修改的指令，注意该操作不可逆。
...
不过如果您使用熟练，也可以可以考虑做定制化的，并且我推荐将 `pull, add, commit` 这三个指令合并在一起，这在个人开发时经常使用。
