...
`Docker` 内置了若干驱动，通常被称作原生驱动或者本地驱动。例如 `Bridge Driver, Host Driver, Overlay Driver, MacVLan Driver, IPVLan Driver, None Driver...`。每个驱动负责创建其上所有网络资源的创建和管理。
...
<span style="text-emphasis:filled red;">这里感觉不对，这部分跳过以后在处理，待补充...</span>

<span style="filter:blur(0.5em);">这种网络模型还是比较有用的，我们需要做一些准备...</span>

```shell
<span style="filter:blur(0.5em);">x # 验证 Overlay 网络模型(准备阶段)# >>> begin: 宿主主机1 >>># 检查本机主机的 Docker 服务是否启动$ sudo systemctl status docker# 拉取镜像文件$ sudo docker pull nginx:1.23.3$ sudo docker image lsREPOSITORY   TAG       IMAGE ID       CREATED         SIZEnginx        1.23.3    ac232364af84   18 months ago   142MB# 开放本机主机的 2377 端口(如果有云服务器厂家面板可能就需要在上面的控制面板上同步开放)$ sudo ufw allow 2377Rules updatedRules updated (v6)# 初始化集群$ sudo docker swarm init --advertise-addr <这里填宿主主机1的公网ip>Swarm initialized: current node (sgu1bhcjokeg3rfsf01eso) is now a manager. # 集群已初始化, sgu1bhcjokeg3rfsf01eso 现在是管理器To add a worker to this swarm, run the following command: # 要向集群中添加工作节点(工作主机), 请运行以下的命令(就是把下面这个命令交给另外一台宿主主机执行即可, 您的输出可能和我不一样, 请正确复制)    docker swarm join --token  Swarm令牌 宿主主机1的公网ip:2377 # 这里给出的是一串 Swarm 令牌, 千万别被泄漏了To add a manager to this swarm, run 'docker swarm join-token manager' and follow the instructions.# 查看当前集群节点列表$ sudo docker node lsID                            HOSTNAME    STATUS    AVAILABILITY   MANAGER STATUS   ENGINE VERSIONsgu1bhcjokeg3rfsf01eso    *   alyubuntu   Ready     Active         Leader           27.1.1# 查看 Swarm 令牌(如果忘记了可以用这个指令进行查看)$ sudo docker swarm join-token workerTo add a worker to this swarm, run the following command:    docker swarm join Swarm令牌 宿主主机1的公网ip:2377# <<< end <<<# >>> begin: 宿主主机2 >>># 检查本机主机的 Docker 服务是否启动$ sudo systemctl status docker# 加入 Swarm 网络$ sudo docker swarm join --token <填入您之前获取到的Swarm令牌> <宿主主机1的公网ip:2377>This node joined a swarm as a worker.# 可以看到工作节点无法查看所有节点列表$ sudo docker node lsError response from daemon: This node is not a swarm manager. Worker nodes can't be used to view or modify cluster state. Please run this command on a manager node or promote the current node to a manager.# <<< end <<<# >>> begin: 宿主主机1 >>># 可以看到管理节点看得到所有节点的信息, * 代表管理节点, 这里显示了 节点id, 节点名称, 节点状态, 可用性, 管理状态, Docker(引擎)版本$ sudo docker node lsID                            HOSTNAME        STATUS    AVAILABILITY   MANAGER STATUS   ENGINE VERSIONo9p0syolrotpzxbmb9x95l        VM-8-9-centos   Ready     Active                          26.1.4sgu1bhcjokeg3rfsf01eso *      alyubuntu       Ready     Active         Leader           27.1.1# <<< end <<<# >>> begin: 宿主主机1 >>># 然后在 宿主主机1 上创建一个 Overlay 网络$ sudo docker network create --driver overlay --attachable my_overlay_network # --attachable 选项创建可手动附加的 Overlay 网络# 可以看到出现了一个 Overlay 网卡$ sudo docker network lsNETWORK ID     NAME                 DRIVER    SCOPE4eb8fda8ac   bridge               bridge    local049a87cac0   docker_gwbridge      bridge    local64718df149   host                 host      localr0hu0wxs99   ingress              overlay   swarm46051d9f9b   my_bridge_network    bridge    localxxmdb3za7z   my_overlay_network   overlay   swarma6b697fc48   none                 null      local# <<< end <<<</span>xxxxxxxxxx # 验证 Overlay 网络模型(准备阶段)# >>> begin: 宿主主机1 >>># 检查本机主机的 Docker 服务是否启动$ sudo systemctl status docker# 拉取镜像文件$ sudo docker pull nginx:1.23.3$ sudo docker image lsREPOSITORY   TAG       IMAGE ID       CREATED         SIZEnginx        1.23.3    ac232364af84   18 months ago   142MB# 开放本机主机的 2377 端口(如果有云服务器厂家面板可能就需要在上面的控制面板上同步开放)$ sudo ufw allow 2377Rules updatedRules updated (v6)# 初始化集群$ sudo docker swarm init --advertise-addr <这里填宿主主机1的公网ip>Swarm initialized: current node (sgu1bhcjokeg3rfsf01eso) is now a manager. # 集群已初始化, sgu1bhcjokeg3rfsf01eso 现在是管理器To add a worker to this swarm, run the following command: # 要向集群中添加工作节点(工作主机), 请运行以下的命令(就是把下面这个命令交给另外一台宿主主机执行即可, 您的输出可能和我不一样, 请正确复制)    docker swarm join --token  Swarm令牌 宿主主机1的公网ip:2377 # 这里给出的是一串 Swarm 令牌, 千万别被泄漏了To add a manager to this swarm, run 'docker swarm join-token manager' and follow the instructions.# 查看当前集群节点列表$ sudo docker node lsID                            HOSTNAME    STATUS    AVAILABILITY   MANAGER STATUS   ENGINE VERSIONsgu1bhcjokeg3rfsf01eso    *   alyubuntu   Ready     Active         Leader           27.1.1# 查看 Swarm 令牌(如果忘记了可以用这个指令进行查看)$ sudo docker swarm join-token workerTo add a worker to this swarm, run the following command:    docker swarm join Swarm令牌 宿主主机1的公网ip:2377# <<< end <<<# >>> begin: 宿主主机2 >>># 检查本机主机的 Docker 服务是否启动$ sudo systemctl status docker# 加入 Swarm 网络$ sudo docker swarm join --token <填入您之前获取到的Swarm令牌> <宿主主机1的公网ip:2377>This node joined a swarm as a worker.# 可以看到工作节点无法查看所有节点列表$ sudo docker node lsError response from daemon: This node is not a swarm manager. Worker nodes can't be used to view or modify cluster state. Please run this command on a manager node or promote the current node to a manager.# <<< end <<<# >>> begin: 宿主主机1 >>># 可以看到管理节点看得到所有节点的信息, * 代表管理节点, 这里显示了 节点id, 节点名称, 节点状态, 可用性, 管理状态, Docker(引擎)版本$ sudo docker node lsID                            HOSTNAME        STATUS    AVAILABILITY   MANAGER STATUS   ENGINE VERSIONo9p0syolrotpzxbmb9x95l        VM-8-9-centos   Ready     Active                          26.1.4sgu1bhcjokeg3rfsf01eso *      alyubuntu       Ready     Active         Leader           27.1.1# <<< end <<<# >>> begin: 宿主主机1 >>># 然后在 宿主主机1 上创建一个 Overlay 网络$ sudo docker network create --driver overlay --attachable my_overlay_network # --attachable 选项创建可手动附加的 Overlay 网络# 可以看到出现了一个 Overlay 网卡$ sudo docker network lsNETWORK ID     NAME                 DRIVER    SCOPE4eb8fda8ac   bridge               bridge    local049a87cac0   docker_gwbridge      bridge    local64718df149   host                 host      localr0hu0wxs99   ingress              overlay   swarm46051d9f9b   my_bridge_network    bridge    localxxmdb3za7z   my_overlay_network   overlay   swarma6b697fc48   none                 null      local# <<< end <<<
```

<span style="filter:blur(0.5em);">这里稍微解释一下，第一个宿主主机就是管理节点，后续加入的宿主主机就是工作节点，可以使用 `sudo docker node ls` 查看当前所有的节点，可以查看到节点列表。</span>

> <span style="filter:blur(0.5em);">补充：`Docker Swarm` 中可以有多个管理节点，但通常建议的数量是奇数个，以确保在发生选举时能够避免出现 `split-brain(分裂脑)` 问题。具体来说：</span>
>
> - <span style="filter:blur(0.5em);">最少 1 个管理节点：适用于小型集群或测试环境。</span>
> - <span style="filter:blur(0.5em);">推荐 3 个管理节点：适用于中小型生产环境，能够容忍一个管理节点的故障</span>
> - <span style="filter:blur(0.5em);">5 个管理节点：适用于更大的生产环境，能够容忍两个管理节点的故障</span>
>
> <span style="filter:blur(0.5em);">使用奇数个管理节点可以提高故障容错能力和系统的可用性，如果您需要把一个工作节点升级为管理节点则可以使用 `sudo docker node promote <节点id值>` 指令来进行提升。</span>

```shell
<span style="filter:blur(0.5em);">xxxxxxxxxx # 验证 Overlay 网络模型(实验阶段)# >>> begin: 宿主主机1 >>># 在管理节点上启动分布式服务$ sudo docker service create --name nginx-service --network my_overlay_network -p 8080:80 nginx:1.23.36q0e0jgfldgw5q6z5g32dhfxoverall progress: 1 out of 1 tasks 1/1: running   [==================================================>] verify: Service 6q0e0jgfldgw5q6z5g32dhfx converged# 查看分布式服务$ sudo docker service ls # 这里显示的是 服务id, 服务名, 模式, 副本, 镜像, 端口ID             NAME            MODE         REPLICAS   IMAGE          PORTS6q0e0jgfldg    nginx-service   replicated   1/1        nginx:1.23.3   *:8080->80/tcp# <<< end <<<# >>> begin: 宿主主机2 >>># 下载镜像文件$ sudo docker pull busybox:1.35# 启动容器进行测试, 看看能不能访问到 Nginx$ sudo docker run -it --network my_overlay_network busybox:1.35 sh# 测试是否能成功访问 Nginx/ # telnet 47.121.195.60 8080Connected to 47.121.195.60GET /<!DOCTYPE html><html><head><title>Welcome to nginx!</title><style>html { color-scheme: light dark; }body { width: 35em; margin: 0 auto;font-family: Tahoma, Verdana, Arial, sans-serif; }</style></head><body><h1>Welcome to nginx!</h1><p>If you see this page, the nginx web server is successfully installed andworking. Further configuration is required.</p><p>For online documentation and support please refer to<a href="http://nginx.org/">nginx.org</a>.<br/>Commercial support is available at<a href="http://nginx.com/">nginx.com</a>.</p><p><em>Thank you for using nginx.</em></p></body></html>Connection closed by foreign host# <<< end <<<</span>xxxxxxxxxx # 验证 Overlay 网络模型(实验阶段)# >>> begin: 宿主主机1 >>># 在管理节点上启动分布式服务$ sudo docker service create --name nginx-service --network my_overlay_network -p 8080:80 nginx:1.23.36q0e0jgfldgw5q6z5g32dhfxoverall progress: 1 out of 1 tasks 1/1: running   [==================================================>] verify: Service 6q0e0jgfldgw5q6z5g32dhfx converged# 查看分布式服务$ sudo docker service ls # 这里显示的是 服务id, 服务名, 模式, 副本, 镜像, 端口ID             NAME            MODE         REPLICAS   IMAGE          PORTS6q0e0jgfldg    nginx-service   replicated   1/1        nginx:1.23.3   *:8080->80/tcp# <<< end <<<# >>> begin: 宿主主机2 >>># 下载镜像文件$ sudo docker pull busybox:1.35# 启动容器进行测试, 看看能不能访问到 Nginx$ sudo docker run -it --network my_overlay_network busybox:1.35 sh# 测试是否能成功访问 Nginx/ # telnet 47.121.195.60 8080Connected to 47.121.195.60GET /<!DOCTYPE html><html><head><title>Welcome to nginx!</title><style>html { color-scheme: light dark; }body { width: 35em; margin: 0 auto;font-family: Tahoma, Verdana, Arial, sans-serif; }</style></head><body><h1>Welcome to nginx!</h1><p>If you see this page, the nginx web server is successfully installed andworking. Further configuration is required.</p><p>For online documentation and support please refer to<a href="http://nginx.org/">nginx.org</a>.<br/>Commercial support is available at<a href="http://nginx.com/">nginx.com</a>.</p><p><em>Thank you for using nginx.</em></p></body></html>Connection closed by foreign host# <<< end <<<
```

> <span style="filter:blur(0.5em);">注意：这里您肯定会注意到使用了一个没见过的指令 `sudo docker service`，这个指令其实就是用来创建分布式架构服务的，您先这么用这，以后有机会再来详细解释。</span>

## 9.8.用容器化技术做实际开发

实际项目的话使用 `Docker` 容器开发就比较简单了，只需要拉取一个 `Ubuntu` 镜像，然后在内部和普通操作系统一样进行开发就行，不过既然有这么多的镜像，如果我们使用 `Docker` 技术来组织一个架构会怎么样？整个系统的可迁移程度就会提高，并且可以随时对资源进行把控！

我们直接朝我们之前提到的一个最为完整的架构来部署一个实际项目，这个项目就是我写的一个模拟微信的一个中小型项目。这个项目涉及到的技术有很多，在架构上是靠近微服务架构的，不过篇幅有点多，我会另外写一篇博文，到时候我会把源码和使用 `Docker` 架构的模拟思路都提供给您...
