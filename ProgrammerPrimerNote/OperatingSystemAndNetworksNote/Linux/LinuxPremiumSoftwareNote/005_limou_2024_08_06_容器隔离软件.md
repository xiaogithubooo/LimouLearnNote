...
# >>> begin: 宿主主机1 和 宿主主机2 都执行以下的命令 >>>
# 检查本机主机的 Docker 服务是否启动
$ sudo systemctl status docker

# 开放本机主机的 2377 端口(如果有云服务器厂家面板可能就需要在上面的控制面板上同步开放)
$ sudo ufw allow 2377
Rules updated
Rules updated (v6)
# <<< end <<<
...
To add a worker to this swarm, run the following command: # 要向集群中添加工作节点(工作主机), 请运行以下的命令(就是把下面这个命令交给另外一台宿主主机执行即可, 您的输出可能和我不一样, 请正确复制)
    ...
    docker swarm join --token  Swarm令牌 宿主主机1的公网ip:2377 # 这里给出的是一串 Swarm 令牌, 千万别被泄漏了
...
sgu1bhcjokeg3rfsf01eso    *   alyubuntu   Ready     Active         Leader           27.1.1

# 查看 Swarm 令牌(如果忘记了可以用这个指令进行查看)
$ sudo docker swarm join-token worker

To add a worker to this swarm, run the following command:

    docker swarm join Swarm令牌 宿主主机1的公网ip:2377
# <<< end <<<

# >>> begin: 宿主主机2 >>>
# 加入 Swarm 网络
$ sudo docker swarm join --token Swarm令牌 宿主主机1的公网ip:2377
