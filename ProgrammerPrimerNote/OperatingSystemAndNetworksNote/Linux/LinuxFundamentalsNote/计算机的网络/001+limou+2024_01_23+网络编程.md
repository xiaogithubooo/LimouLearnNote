...
有了端口号就可以确定一台拥有 `IP` 地址的主机上的某一个网络进程，这样两台主机就可以点对点的在两个进程之间进行交互。
...
in_addr_t inet_addr(const char *ip); // 将主机字节序 ip 从点分十进制字符串形式转换为网络字节序, 不过推荐使用 int inet_pton(int af, const char *src, void *dst) 您了解下即可
char *inet_ntoa(struct in_addr in); // 将网络字节序的 ip 从二进制形式转换回点分十进制字符串, 不过也是推荐使用 const char *inet_ntop(int af, const void *src, char *dst, socklen_t size)
```
