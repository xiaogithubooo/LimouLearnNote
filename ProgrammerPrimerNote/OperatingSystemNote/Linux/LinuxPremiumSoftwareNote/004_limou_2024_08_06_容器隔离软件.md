...
-   `docker push` 默认从 `Docker Hub` 推送镜像文件
...
-   `docker images ls` 查看当前本地存储库已有的镜像文件，带上 `--digests` 显示镜像的摘要信息，`-q` 只显示镜像 `ID`
-   `docker images inspect <repository>:<tag>` 查看镜像详细信息
-   `docker images history` 查看镜像历史，提供每一层的详细信息
-   `docker images build` 通过 `dockerfile` 制作镜像文件
-   `docker images rm <repository>:<tag>` 删除镜像文件
-   `docker images tag <old-repository>:<old-tag>/<existing-image-id> <new-repository>:<new-tag>` 修改镜像文件的名称和标签，类似 `git` 做一个标记
-   `docker images save` 将镜像文件保存为归档文件
-   `docker images load` 将归档文件加载为镜像文件
-   `docker images export` 将容器文件系统导出为快照归档文件
-   `docker images import` 将快照归档文件导入创建出新的镜像
-   `docker images prune` 删除不使用的镜像（没有标签的容器）

### 6.3.3.镜像容器

#### 6.3.3.1.镜像容器原理

容器是镜像文件的一份实例化，本质就是操作系统的一个进程，容器有 `初建(created), 运行(running), 停止(stopped), 暂停(paused), 删除(deleted)` 五种状态。
...
-   `docker ps` 累出容器信息列表
    ...
    -   `-q` 静默模式，只显示容器编号
    -   `-s` 显示总的文件大小
-   `(初建)docker create` 创建容器后，不立即启动运行，容器进入初建状态
-   `(运行)docker start` 容器转为运行状态
-   `(初建+运行)docker run --name <指令容器名称> [-d] [--rm] [--it] [-p <本地端口:容器端口>] [-v <本地目录:容器目录>] <镜像名称:镜像版本> [bash]` 启动容器
    -   `--name <指令容器名称>` 指定容器名称
    -   `-d` 以后台模式启动容器
    -   `-p <本地端口:容器端口>` 做端口映射
    -   `-v <本地目录:容器目录>` 做目录映射, 两端共享目录下所有数据
    -   `--rm` 可以在容器退出时自动删除容器，适合做临时测试
    -   `--it` 表示保持标准输入并且为容器分配一个类似本地操纵的伪终端
    -   `<镜像名称:镜像版本>` 要启动的目标镜像文件
    -   `bash` 表示启动一个 `bash` 程序
-   `(停止)docker stop <容器id>/<容器名称>[, ...]` 停止运行容器，完全关闭并且进行清理，如果超过过时间就会销毁
-   `(停止)docker kill` 容器在故障（死机）时，执行 `kill`（断电），容器转入停止状态，这种操作容易丢失数据，除非必要，否则不建议使用
-   `(开始暂停)docker pause` 容器进入暂停状态
-   `(取消暂停)docker unpause` 取消暂停状态，容器进入运行状态
-   `(禁止)docker rm` 删除容器，容器转入删除状态
-   `docker restart` 重启容器，容器转入运行状态

这里面很多指令还挺简单的，这里我简单实验几个指令。首先我们先来使用别人的镜像文件，再来考虑自己做一个镜像文件（我们之前做的镜像文件充其量只是拷贝了一下，修改了 `tag` 标签信息而已，不够实际）。
...
# 运行镜像文件
$ sudo docker run --name my_nginx --rm --it -p 8086:80 nginx:1.23.3 bash
...
# 不过我们就不做配置文件了, 因为这个环境几乎没有一个常用编辑器
...
*   **描述**：当容器中的应用程序使用的内存超过了主机系统分配给该容器的内存限制时，会触发OOM事件。
*   **示例**：假设一个容器被分配了1GB的内存限制，如果容器内的应用程序尝试申请超过1GB的内存，操作系统会触发OOM事件。
*   处理：
    *   容器会被强制关闭。
    *   关闭容器的不是Docker Daemon，而是宿主机的操作系统。因为容器实际上是运行在宿主机操作系统上的一组进程。
    *   宿主机通过cgroups（控制组）对这些进程设定资源上限，当进程申请的资源达到上限时，宿主机内核会触发OOM事件并关闭相关进程。

### 8.1.2.禁用 OOM

*   **描述**：如果用户不希望容器在内存耗尽时被关闭，可以使用`--oom-kill-disable`参数来禁用OOM-Killer。

*   注意事项

    ：

    *   如果使用`-m`参数设置了容器的内存上限，当容器达到内存资源上限时，主机不会关闭容器，但也不会继续分配资源。
    *   此时，容器将处于“hung”状态，即无法继续执行，但也不会被强制关闭。

### 8.1.3.没有内存限制

*   **描述**：如果用户使用了`--oom-kill-disable`，但没有使用`-m`设置内存上限，容器将尽可能多地使用主机的内存资源。
*   **效果**：容器可以使用主机的所有可用内存，直到主机内存耗尽。

## 8.2.异常终止

*   **描述**：每个容器内部都有一个Init进程，其他所有进程都是其子进程。容器的运行依赖于Init进程。
*   退出机制：
    *   如果一个子进程因某种原因退出，Init进程也会同步退出，最终导致整个容器关闭。
    *   Docker无法区分进程的退出是正常还是异常。
*   重启策略：
    *   当容器关闭时，Docker Daemon会尝试将容器从“Stopped”状态转为“Running”状态。
    *   只有设置了`--restart`参数的容器，Docker Daemon才会尝试重启，否则容器将保持停止状态。
