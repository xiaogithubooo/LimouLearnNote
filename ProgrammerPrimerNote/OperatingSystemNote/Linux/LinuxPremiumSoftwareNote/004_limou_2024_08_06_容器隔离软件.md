...
-   `docker images` 查看当前本地存储库已有的镜像文件，带上 `--digests` 显示镜像的摘要信息，`-q` 只显示镜像 `ID`，如果加上 ` inspect <名称:版本>` 则可以查看本地
-   `docker tag <old-repository>:<old-tag>/<existing-image-id> <new-repository>:<new-tag>` 修改镜像文件的名称和标签，类似 `git` 做一个标记
-   `docker rmi <repository>:<tag>` 删除镜像文件
-   `docker search` 查看当前远端存储库 `Docker Hub` 中已有公开的镜像文件

其中，如果是需要登录的镜像地址，就需要到镜像仓库中创建一个账户，然后才可以开始实践有关登录的指令。
...
# 另外一种拉取镜像的方法
$ sudo docker pull nginx@sha256:f5747a42e3adcb3168049d63278d7251d91185bb5111d2563d58729a5c9179b0 # 这串字符类源于 dockerhub 的 tag 界面中, 下面有给出图解
...
>   补充：如果是自己部署或公司部署的私人镜像源，则需要补全网址，例如 `docker pull myprivateregistry.com/myimage:latest`，后续的 `push` 指令也是类似。
...
```shell
# 给自己的镜像文件设置名称和标签, 并尝试使用 rmi 和 push
# 查看当前已有的镜像文件
$ docker images
REPOSITORY    TAG       IMAGE ID       CREATED         SIZE
nginx         1.23.4    a7be6198544f   15 months ago   142MB
...
```shell
# 启动和停止容器
docker run [-p <本地端口:容器端口>] [-v <本地目录:容器目录>] --name <指令容器名称> [-d] <镜像名称:镜像版本>
# -p <本地端口:容器端口> 做端口映射
...
docker stop <容器id>/<容器名称>[, ...] # 容器 id 可以在后面 ps 中显示出来
```

```shell
# 查看容器
docker ps [-a] [-q] [--filter ...] [--format] # 默认查找运行中的容器
# -a 显示所有的容器, 包括未运行的
# -f/--filter 根据条件过滤显示的内容
# --format 指定返回值的模板文件, 如 json 或者 table 格式返回
# -l 显示 latest 的容器, 也就是最新创建的容器
# -n 列出最近创建的 n 个容器
# --no-trunc 不截断输出
# -q 静默模式, 只显示容器编号
# -s/--size 显示容器的总文件大小, 包括写入的文件大小
```
