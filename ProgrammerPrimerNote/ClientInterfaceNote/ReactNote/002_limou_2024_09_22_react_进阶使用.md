...
而关于 `AntV` 图表库的使用，可以查看 [AntV 图表库的链接](https://ant-design-charts.antgroup.com/manual/getting-started)，例如下面这个例子。
...
先下载好 `npm i @umijs/plugin-openapi`，然后在 `antd` 中引入了相关 `OpenAPI` 的配置（如下内容）。
...
还需要在 `package.json` 的 `scripts` 中增加一个命令（同样如果 `antd` 有配置就不要）。
...
> 补充：不过我的 `antd` 版本写的是 `"openapi": "max openapi"`，并且运行后就会导致页面出错...
