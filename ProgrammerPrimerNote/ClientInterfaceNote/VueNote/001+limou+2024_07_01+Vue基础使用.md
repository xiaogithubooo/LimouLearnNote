...
`Ajax` 的全称为 `Asynchronous JavaScript And XML`，也就是异步的 `JS` 和 `XML`。其中异步其实是指不需要重新加载整个页面的情况下，就可以实时更新数据，这比一些早期依靠后端模板渲染的网站要更加高效，本质是通过更新部分的网页让渲染更加快速，并且这种方式还可以让用户等待页面的部分内容更新的时候，继续浏览本页面，也提高了用户体验。
